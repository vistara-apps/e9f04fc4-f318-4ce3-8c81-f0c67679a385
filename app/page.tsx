...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-bg">
      ...
      {/* Hero Section */}
      <section className={`relative overflow-hidden border-b border-[var(--color-border)] ${isRetro ? 'scanlines' : ''}`}>
        <div className="absolute inset-0 bg-gradient-to-br from-accent/10 via-transparent to-transparent" />
        <div className={`absolute inset-0 opacity-30 ${isRetro ? 'grid-bg' : ''}`} />
          ...
          <div className="text-center space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-12 max-w-4xl mx-auto">
              <div className={`glass-effect rounded-lg p-6 ${isRetro ? 'retro-card' : ''}`}>
                <div className={`text-3xl font-bold text-accent ${isRetro ? 'neon-text' : ''}`}>
                  $24.5M
                </div>
                <div className="text-sm text-fg/60 mt-1">Total Value Locked</div>
              </div>
              <div className={`glass-effect rounded-lg p-6 ${isRetro ? 'retro-card' : ''}`}>
                <div className={`text-3xl font-bold text-success ${isRetro ? 'neon-text' : ''}`}>
                  18.4%
                </div>
                <div className="text-sm text-fg/60 mt-1">Avg APY</div>
              </div>
              <div className={`glass-effect rounded-lg p-6 ${isRetro ? 'retro-card' : ''}`}>
                <div className={`text-3xl font-bold text-fg ${isRetro ? 'text-glow-primary' : ''}`}>
                  12
                </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-12">
        <div className="grid md:grid-cols-4 gap-6">
          <div className={`glass-effect rounded-lg p-6 hover:glow-accent transition-all duration-300 ${
            ...
            <Zap className={`w-8 h-8 text-accent mb-4 ${isRetro ? 'glow-accent' : ''}`} />
            <h3 className={`text-lg font-semibold mb-2 ${isRetro ? 'text-glow-accent' : ''}`}>
              Auto-Compound
            </h3>
          ...
          <div className={`glass-effect rounded-lg p-6 hover:glow-accent transition-all duration-300 ${
            ...
            <Shield className={`w-8 h-8 text-accent mb-4 ${isRetro ? 'glow-accent' : ''}`} />
            <h3 className={`text-lg font-semibold mb-2 ${isRetro ? 'text-glow-accent' : ''}`}>
              Risk Management
            </h3>
          ...
          <div className={`glass-effect rounded-lg p-6 hover:glow-accent transition-all duration-300 ${
            ...
            <TrendingUp className={`w-8 h-8 text-accent mb-4 ${isRetro ? 'glow-accent' : ''}`} />
            <h3 className={`text-lg font-semibold mb-2 ${isRetro ? 'text-glow-accent' : ''}`}>
              Optimized Yields
            </h3>
          ...
          <div className={`glass-effect rounded-lg p-6 hover:glow-accent transition-all duration-300 ${
            ...
            <Coins className={`w-8 h-8 text-accent mb-4 ${isRetro ? 'glow-accent' : ''}`} />
            <h3 className={`text-lg font-semibold mb-2 ${isRetro ? 'text-glow-accent' : ''}`}>
              Tokenized LPs
            </h3>
        ...
        <div className="flex gap-2 border-b border-[var(--color-border)] mb-8">
          <button
            ...
            className={`px-6 py-3 font-medium transition-all duration-200 border-b-2 ${
              activeTab === 'vaults'
                ? `border-accent text-accent ${isRetro ? 'neon-text' : ''}`
